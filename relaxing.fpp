...
#ifdef _ALPHA
      common /relaxingalign/ baj,vt,res,u1,vtf,vtfstore,c,u1store
!dec$ psect /relaxingalign/ align=page
#endif
...
c parallelizations.
#ifdef _ALPHA
      common /dpvalign/ vk,vij,u1,u2,flz,flx
!dec$ psect /dpvalign/ align=page
#endif
...
#ifdef COLD
c$omp parallel default(private) firstprivate(k,lk)
c$omp& shared(flx,vt,indxr,c,indxc,res,vij,cold)
#else
c$omp parallel default(private) firstprivate(k,lk)
c$omp& shared(flx,vt,indxr,c,indxc,res,vij)
#endif
c$omp do
      do j=1,ng2
#ifdef COLD         
...
#ifdef COLD
c$omp parallel default(private) firstprivate(k,lk)
c$omp& shared(flx,vt,indxr,c,indxc,res,cold)
#else
c$omp parallel default(private) firstprivate(k,lk)
c$omp& shared(flx,vt,indxr,c,indxc,res)
#endif
#ifdef COLD         
c*$* assert do (concurrent)
c$omp do
...
c parallelizations.
#ifdef _ALPHA
      common /rhodpvalign/ vk,vij,u1,u2,flz,flx
!dec$ psect /rhodpvalign/ align=page
#endif
...
c*$* assert do (concurrent)
c$omp parallel default(private) firstprivate(k,lk)
c$omp& shared(cold,flx,vt,indxr,indxc,res)
c$omp do
      do j=1,ng2
         jp=mod(j,ng2)+1
         jm=mod(j-2+ng2,ng2)+1
...
c$omp parallel default(private) shared(u,v,flz)
c*$* assert do (concurrent)
c$omp do
      do j=1,ng2
         do i=1,ng1
...
c$omp parallel default(private) shared(baj,k,indx,def)
...
cmydist pdcache(*,*,block)
      integer i,j,ndim,nf,ip,jp,kp,im,jm,km,l, ik,ii
      integer lastk
      save lastk
      data lastk /0/
#ifdef _SX5
...
c$omp parallel default(private) firstprivate(ik,k,nfluidcmp)
c$omp& shared(rootg,baj,ul,u,indxu)
c$omp do
      do j=1,ng2
!cdir vector
         do i=1,ng1
...
c$omp parallel default(private) firstprivate(nfluidcmp,ik,k)
c$omp& shared(rootg,defp,baj,vt1,u,indxu,vt)
!cdir nodep
c$omp do
      do j=1,ng2
!cdir vector, nodep
...
cc$doacross local(i,j)
#ifdef COLD
c$omp parallel default(private) shared(rootg,u,vt,k,cold)
#else
c$omp parallel default(private) shared(rootg,u,vt,k)
#endif
c$omp do
      do j=1,ng2
         do i=1,ng1
...
c$omp parallel default(private) firstprivate(k)
c$omp& shared(defp,baj,u,c,indxc,indx)
c$dir prefer_parallel_ext         
cc$doacross local(i,j)
c$omp do
      do j=1,ng2
