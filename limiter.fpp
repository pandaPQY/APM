...
c cell
      ...
      include 'cold.fi'
#else
c fool OMP to accept a nonexistent variable
      real cold
#endif
#ifdef GMETRIC
      include 'gmetric.fi'
...
c the distribution of the second tmp can be problematic.
      ...
      do i=1,ng1
         ...
         rhol= 1.5*u(1,i,j,k)
c if any of the six neighboring cells is more than 1.5x as dense,
c revert to TVD               
         rhonn=max(u(1,i,j,km), u(1,i,j,kp)
     &         ,u(1,i,jm,k),u(1,i,jp,k)
     &         ,u(1,im,j,k),u(1,ip,j,k)
     &              )
