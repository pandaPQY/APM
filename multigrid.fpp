...
#ifdef FIXEDGRID
      include 'relaxgl.fi'
      real tmpfft(ng1+2,ng2,ng3),pi,xk,yk,zk,akernel
      complex ctmpfft(ng1/2+1,ng2,ng3)
      equivalence(tmpfft,ctmpfft)
      integer i,j,k

      if (nx .ne. ng1 .or. nx .ne. ng2 .or. nz .ne. ng3) then
         write(*,*) 'fft gravity: wrong arguments'
         stop
      endif
      pi=4*atan(1.)
      do k=1,ng3
         do j=1,ng2
            do i=1,ng1
               tmpfft(i,j,k)=rhs(i,j,k)
            enddo
         enddo
      enddo
      call fft3(tmpfft,ng3,1)
      do k=1,ng3
         do j=1,ng2
            do i=1,ng1/2+1
               xk=2*pi*(i-1)/ng1
               ...
               if (i*j*k>1) then
                  ...
                  ctmpfft(i,j,k)=ctmpfft(i,j,k)/akernel
               endif
            enddo
         enddo
      enddo
